...
Процедура ВыгрузитьВерсииВЛокальныйРепозиторий(Знач Хранилище, КоличествоВерсийВыгружаемыхЗаРаз = 0) Экспорт
	...
	Попытка
		...
		Для Каждого ДанныеВерсии из ДанныеПоВерсиям Цикл 
			...
			ВыгрузкитьВерсиюВЛокальныйРепозиторий(Хранилище, ДанныеВерсии.Версия, ДанныеВерсии);
...
Процедура ВыгрузкитьВерсиюВЛокальныйРепозиторий(Хранилище, Версия, Знач ДанныеВерсии= Неопределено)
	...
	ПолучитьВерсиюИзХранилища(Хранилище, ДанныеВерсии);
	
		
	ЛогОтладка("Подготовка каталога и конфигурирование пользователя");
	
	Git.СконфигурироватьИмяПользователя(Хранилище.ЛокальныйРепозиторийАдрес, ДанныеВерсии.ИмяПользователя);
	Git.СконфигурироватьEmailПользователя(Хранилище.ЛокальныйРепозиторийАдрес, ДанныеВерсии.EmailПользователя);
	...
	УстановитьВерсиюВВерсию(Версия);
	
	ВыгрузитьАктуальныйCF(Хранилище, Версия); // Выгружаем cf до коммита, т.к. cf может выгружаться как раз в репо
	УстановитьНовуюВерсиюКонфигурации(Хранилище, Версия.ВерсияКонфигурации); // Аналогично скрипт при смене запускает до коммита
	
	ЛогОтладка("Включение отслеживания и коммит");
	
	Git.ОтслеживатьВсеФайлыВКаталоге(Хранилище.ЛокальныйРепозиторийАдрес);
	
	Git.СохранитьСписокИзменений(Хранилище.ЛокальныйРепозиторийАдрес, Хранилище, Версия);
	
	Git.ВыполнитьКоммит(Хранилище.ЛокальныйРепозиторийАдрес,
	ПредставлениеКомита + Символы.ПС + ДанныеВерсии.Комментарий,
	ДанныеВерсии.ДатаСоздания,
	Хранилище);
	
	
	УстановитьПризнакВыгруженностиВерсииВЛокальныйРепозиторий(Версия);
...
Процедура ПолучитьВерсиюИзХранилища(Хранилище, ДанныеВерсии) Экспорт 
