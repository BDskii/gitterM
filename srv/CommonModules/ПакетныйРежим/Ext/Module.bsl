...
Функция ПолучитьТаблицуВерсийХранилища(Знач Хранилище, НачинаяСВерсии = Неопределено, ЗаканчиваяНаВерсии = Неопределено) Экспорт
...
Функция ПолучитьТаблицуВерсийИзТабличногоДокумента(ТабличныйДокумент)
	...
	Пока ТабличныйДокумент.ВысотаТаблицы >= НачинаяСоСтроки Цикл
		...
		Если ОбластьРезультат = Неопределено Тогда //на as5 английская локаль со всеми вытекающими
			ОбластьРезультат = ТабличныйДокумент.НайтиТекст("Version:",,ОбластьПоиска, Истина, Истина, Истина, Ложь);
		КонецЕсли;	
		...
		Если ОбластьРезультат <> Неопределено Тогда
			...
			Если СтрНайти(ВремяСоздания, "AM") <> Неопределено или СтрНайти(ВремяСоздания, "PM") <> Неопределено Тогда 
				...
				НоваяСтрока.ДатаСоздания = Дата(ДатаСоздания + " " + ВремяСоздания) + (ДопЧасы*60*60);
			Иначе 	
