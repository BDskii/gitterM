...
Процедура ВыполнитьИндексированиеИКоммит(Знач ЛокальныйРепозиторийАдрес,
	...
	УдалитьФайлПоВозможности(КомандныйФайл_Индекс);
...
Процедура СохранитьСписокИзменений(Знач ЛокальныйРепозиторийАдрес, Знач Хранилище, Знач пВерсия) Экспорт
	...
	ОтслеживатьВсеФайлыВКаталоге(ЛокальныйРепозиторийАдрес);
	...
	РедактируемыйОбъект = пВерсия.ПолучитьОбъект();
	...
	Для НомерСтроки = 0 по СтрЧислоСтрок(Текст) Цикл 
		...
		//Если ЗначениеЗаполнено(имяКаталога) и ЗначениеЗаполнено(ТекСтрока) 
		//	и Лев(имяКаталога,1) = "/" и Лев(ТекСтрока,1) <> "/" тогда
		//	
		//	ТекСтрока = "/" + ТекСтрока;
		//	
		//КонецЕсли; 	
