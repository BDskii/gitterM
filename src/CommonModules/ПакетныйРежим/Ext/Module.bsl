...
Функция ПолучитьТаблицуВерсийХранилища(Хранилище, НачинаяСВерсии = Неопределено, ЗаканчиваяНаВерсии = Неопределено, ПолучатьИзменения = Ложь) Экспорт
	...
	СтруктураПараметров.НачинаяСВерсии = НачинаяСВерсии;
	СтруктураПараметров.ЗаканчиваяНаВерсии = ЗаканчиваяНаВерсии;
	СтруктураПараметров.ПолучатьИзменения = ПолучатьИзменения;
...
Процедура ПолучитьТаблицуВерсийИзTool1CD(СтруктураПараметров)
	...
	КаталогРаспаковщика = ПакетныйРежим.РаспаковатьХранящейсяМакет("Tool_1CD");
	файлРаспаковщика = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогРаспаковщика, "Tool1CD.exe");
	...
	КоммандаВыгрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомманды,
	файлРаспаковщика, СтруктураПараметров.Хранилище.ХранилищеАдрес, КаталогВыгрузки);	
	
	ВыполнитьКоманду(КаталогВременныхФайлов(), КоммандаВыгрузки, истина);
...
Функция РаспаковатьХранящейсяМакет(ИмяМакета) Экспорт
	
	КаталогРаспаковщика = ПолучитьИмяВременногоФайла("");
	СоздатьКаталог(КаталогРаспаковщика);
		
	ДДРаспаковщика = ПолучитьОбщийМакет(ИмяМакета);
	ЧтениеZipФайла = новый ЧтениеZipФайла(ДДРаспаковщика.ОткрытьПотокДляЧтения());
	
	ЧтениеZipФайла.ИзвлечьВсе(КаталогРаспаковщика, РежимВосстановленияПутейФайловZIP.Восстанавливать);
	
	Возврат КаталогРаспаковщика;
