...
Процедура ЗагрузитьНовыеВерсии(Знач Хранилище) Экспорт
	...
	ТаблицаВерсий = ПакетныйРежим.ПолучитьТаблицуВерсийХранилища(Хранилище, НомерПоследнейВерсииВХранилище + 1,,Истина);
	...
	Для Каждого ТекущаяСтрока Из ТаблицаВерсий Цикл
		...
		НоваяВерсия.ИзмененныеОбъекты.Загрузить(ТекущаяСтрока.Изменения);
...
Процедура ВыгрузитьВерсииВЛокальныйРепозиторий(Знач Хранилище, КоличествоВерсийВыгружаемыхЗаРаз = 0) Экспорт
	...
	ВерсииДляВыгрузки = результатЗапроса.Выгрузить().ВыгрузитьКолонку("Версия");
	ДанныеПоВерсиям = Справочники.ВерсииКонфигурацийХранилища.СобратьДанныеПоВерсиям(ВерсииДляВыгрузки);
	
	Попытка
		
		Для Каждого ДанныеВерсии из ДанныеПоВерсиям Цикл 
			#Если Клиент Тогда
			...
			ВыгрузкитьВерсиюВЛокальныйРепозиторий(ДанныеВерсии.Версия, ДанныеВерсии);
			
			Справочники.Метрики.СобратьМетрикиПоВерсии(ДанныеВерсии.Версия);	
...
Процедура ВыгрузкитьВерсиюВЛокальныйРепозиторий(Версия, Знач ДанныеВерсии = Неопределено)
	...
	ПредставлениеКомита = "Номер в хранилище:" + ДанныеВерсии.НомерВерсииВХранилище + " Порядковый номер:" + ДанныеВерсии.НомерВерсии;
	
	началоВыгрузкиОбщая = ТекущаяУниверсальнаяДатаВМиллисекундах();
	
	ЛогИнформация("Версия: " + ДанныеВерсии.НомерВерсииВХранилище + ", " + ДанныеВерсии.ИмяПользователя + ": " + ДанныеВерсии.Комментарий);
	
	ПолучитьВерсиюИзХранилища(ДанныеВерсии);
	
	ЛогОтладка("Конфигурирование пользователя");
	
	Git.СконфигурироватьИмяПользователя(ДанныеВерсии.Хранилище.ЛокальныйРепозиторийАдрес, ДанныеВерсии.ИмяПользователя);
	Git.СконфигурироватьEmailПользователя(ДанныеВерсии.Хранилище.ЛокальныйРепозиторийАдрес, ДанныеВерсии.EmailПользователя);
		
	УстановитьВерсиюВВерсию(ДанныеВерсии.Версия);
	
	ВыгрузитьАктуальныйCF(ДанныеВерсии.Хранилище, ДанныеВерсии.Версия); // Выгружаем cf до коммита, т.к. cf может выгружаться как раз в репо
	
	УстановитьНовуюВерсиюКонфигурации(ДанныеВерсии.Хранилище, ДанныеВерсии.ВерсияКонфигурации); // Аналогично скрипт при смене запускает до коммита
	
	ЛогОтладка("Включение отслеживания и коммит");
	
	Git.ОтслеживатьВсеФайлыВКаталоге(ДанныеВерсии.Хранилище.ЛокальныйРепозиторийАдрес);
	
	Git.СохранитьСписокИзменений(ДанныеВерсии.Хранилище.ЛокальныйРепозиторийАдрес, ДанныеВерсии.Хранилище, ДанныеВерсии.Версия);
	
	Git.ВыполнитьКоммит(ДанныеВерсии.Хранилище.ЛокальныйРепозиторийАдрес,
	ПредставлениеКомита + Символы.ПС + ДанныеВерсии.Комментарий,
	ДанныеВерсии.ДатаСоздания,
	ДанныеВерсии.Хранилище);
	
	
	УстановитьПризнакВыгруженностиВерсииВЛокальныйРепозиторий(ДанныеВерсии.Версия);
	
	затраченоВсего = ТекущаяУниверсальнаяДатаВМиллисекундах() -началоВыгрузкиОбщая;
	ЛогИнформация("Обработана версия " + ДанныеВерсии.НомерВерсииВХранилище + ". " + затраченоВсего + "мс");
	
КонецПроцедуры	

Процедура ПолучитьВерсиюИзХранилища(ДанныеВерсии) Экспорт 
	
	КаталогКонфигурации = ПолучитьКаталогКонфигурации(ДанныеВерсии.Хранилище);
	
	НачалоВыгрузки = ТекущаяУниверсальнаяДатаВМиллисекундах();
	
	ЛогОтладка("Загрузка из хранилища версии " + ДанныеВерсии.НомерВерсииВХранилище);
	
	ПакетныйРежим.ЗагрузитьКонфигурациюИзХранилища(ДанныеВерсии.Хранилище, ДанныеВерсии.НомерВерсииВХранилище);
	
	ЛогОтладка(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Выгружена %1. %2мс",
	ДанныеВерсии.НомерВерсииВХранилище,
	ТекущаяУниверсальнаяДатаВМиллисекундах() -началоВыгрузки));
	
	
	ЛогОтладка("Выгрузка файлов");
	НачалоВыгрузки = ТекущаяУниверсальнаяДатаВМиллисекундах();
	
	ПакетныйРежим.ВыгрузитьКонфигурациюВФайлы(ДанныеВерсии.Хранилище, КаталогКонфигурации);
	
	
	ЛогОтладка(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Выгружены файлы. %1мс",
	ТекущаяУниверсальнаяДатаВМиллисекундах() - началоВыгрузки));
	
	РаспаковатьОбычныеФормы(КаталогКонфигурации, ДанныеВерсии.Хранилище);
...
Процедура РаспаковатьОбычныеФормы(Знач КаталогКонфигурации, Знач Хранилище)
	...
	КаталогРаспаковщика = ПакетныйРежим.РаспаковатьХранящейсяМакет("V8Unpack");
	файлРаспаковщика = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогРаспаковщика, "V8Unpack.exe");
	...
	Для каждого цФайл Из формыКРаспаковке Цикл
		...
		УдалитьФайлПоВозможности(каталогРаспаковки + "\Form");
		УдалитьФайлПоВозможности(цФайл.ПолноеИмя);
	...
	УдалитьФайлПоВозможности(КаталогРаспаковщика);
...
Функция ПолучитьКаталогКонфигурации(Знач Хранилище) Экспорт
	...
	каталогКонфигурации = ПолныйПутьККаталогуКонфигурации(Хранилище);
