...
Процедура ВыполнитьСборМетрикПоВерсии(Версия) Экспорт
	...
	Справочники.Хранилища.УставновитьОтметкуЗанятости(ДанныеВерсии.Хранилище, Истина, "Выгрузка в локальный репозиторий");
	
	ЛогОтладка("--Сбор метрик по " + ДанныеВерсии.Хранилище + "--");
	
	Справочники.Хранилища.ПолучитьВерсиюИзХранилища(ДанныеВерсии.Хранилище, ДанныеВерсии);
	
	СобратьМетрикиПоВерсии(Версия);
	
	ЛогОтладка("--Закончен сбор метрик по " + ДанныеВерсии.Хранилище + "--");
	
	Справочники.Хранилища.УставновитьОтметкуЗанятости(ДанныеВерсии.Хранилище, Ложь);
...
Функция ДобавитьОбнулениеОтсутствующих(Хранилище, Версия, МассивУдаленныхФайлов = Неопределено, ТаблицаМетрик)
	...
	|					ИЛИ ВЫРАЗИТЬ(ДанныеМетрик.Файл КАК СТРОКА(1000)) В (&МассивУдаленныхФайлов))
	|		
	|		СГРУППИРОВАТЬ ПО
